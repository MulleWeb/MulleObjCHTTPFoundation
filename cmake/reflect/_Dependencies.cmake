...
if( NOT MULLE_HTTP_LIBRARY)
   ...
   if( MULLE_HTTP_LIBRARY)
      ...
      # Disable with: `mulle-sourcetree mark mulle-http no-cmakeadd`
      #
      ...
      #
      # Inherit ObjC loader and link dependency info.
      # Disable with: `mulle-sourcetree mark mulle-http no-cmakeinherit`
      #
      # // temporarily expand CMAKE_MODULE_PATH
      get_filename_component( _TMP_MULLE_HTTP_ROOT "${MULLE_HTTP_LIBRARY}" DIRECTORY)
      get_filename_component( _TMP_MULLE_HTTP_ROOT "${_TMP_MULLE_HTTP_ROOT}" DIRECTORY)
      #
      #
      # Search for "DependenciesAndLibraries.cmake" to include.
      # Disable with: `mulle-sourcetree mark mulle-http no-cmakedependency`
      #
      foreach( _TMP_MULLE_HTTP_NAME "mulle-http")
         set( _TMP_MULLE_HTTP_DIR "${_TMP_MULLE_HTTP_ROOT}/include/${_TMP_MULLE_HTTP_NAME}/cmake")
         # use explicit path to avoid "surprises"
         if( EXISTS "${_TMP_MULLE_HTTP_DIR}/DependenciesAndLibraries.cmake")
            unset( MULLE_HTTP_DEFINITIONS)
            list( INSERT CMAKE_MODULE_PATH 0 "${_TMP_MULLE_HTTP_DIR}")
            # we only want top level INHERIT_OBJC_LOADERS, so disable them
            if( NOT NO_INHERIT_OBJC_LOADERS)
               set( NO_INHERIT_OBJC_LOADERS OFF)
            endif()
            ...
            set( NO_INHERIT_OBJC_LOADERS ON)
            #
            include( "${_TMP_MULLE_HTTP_DIR}/DependenciesAndLibraries.cmake")
            #
            list( GET _TMP_INHERIT_OBJC_LOADERS -1 NO_INHERIT_OBJC_LOADERS)
            list( REMOVE_AT _TMP_INHERIT_OBJC_LOADERS -1)
            #
            list( REMOVE_ITEM CMAKE_MODULE_PATH "${_TMP_MULLE_HTTP_DIR}")
            set( INHERITED_DEFINITIONS
               ${INHERITED_DEFINITIONS}
               ${MULLE_HTTP_DEFINITIONS}
               CACHE INTERNAL "need to cache this"
            )
            break()
         else()
            message( STATUS "${_TMP_MULLE_HTTP_DIR}/DependenciesAndLibraries.cmake not found")
         endif()
      endforeach()
   else()
